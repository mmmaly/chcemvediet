...
urlpatterns = patterns(u'',
    ...
    url(_(r'^decide-email/(?P<action>[\w-]+)/(?P<inforequest_id>\d+)/(?P<email_id>\d+)/$'), views.decide_email, name=u'decide_email'),
    ...
    url(_(r'^extend-deadline/(?P<inforequest_id>\d+)/(?P<paperwork_id>\d+)/(?P<action_id>\d+)/$'), views.extend_deadline, name=u'extend_deadline'),
