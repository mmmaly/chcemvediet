...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        def rec(history, inforequest):
            history.inforequest = inforequest
            history.save()
            for action in history.action_set.all():
                for sub_history in action.advanced_to_set.all():
                    rec(sub_history, inforequest)

        for inforequest in orm[u'inforequests.inforequest'].objects.all():
            rec(inforequest.history, inforequest)

        broken = orm[u'inforequests.history'].objects.filter(inforequest=None).all()
        if broken:
            print(u'Warning: %d History instances are missing a reference to Inforequest. They are broken probably. Their primary keys are: %s' % (len(broken), list(o.pk for o in broken)))

    def backwards(self, orm):
        "Write your backwards methods here."

        for inforequest in orm[u'inforequests.inforequest'].objects.all():
            inforequest.history = inforequest.history_set.get(advanced_by=None)
            inforequest.save()

        broken = orm[u'inforequests.inforequest'].objects.filter(history=None).all()
        if broken:
            print(u'Warning: %d Inforequest instances are missing a reference to History. They are broken probably. Their primary keys are: %s' % (len(broken), list(o.pk for o in broken)))
